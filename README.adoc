...
| `hotdog.connections`
| Counter tracking the number of connections
...
| `hotdog.kafka.producer.sending`
| Timer which tracks the amount of time `hotdog` spends trying to send something via librdkafka. Will indicate calls blocking on the asynchronous outbound queue filling up.
